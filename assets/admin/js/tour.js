		...
		ajax.get(post_data).done(function(result) {
			...
			if (result.data.status) {
				//検索時ヒット無し
					...
					$("[name=new_entry]").on("click",function(){
						initAutocomplete(35.680366377806806,139.7671272)
						$("[name=schedule]").val("35.680366377806806@139.7671272");
					})
	...
	call_ajax_edit_init = function (post_data){
		ajax.get(post_data).done(function(result) {
			...
			if (result.data.status) {
				...
				var g_data = result.data.schedule.split("@")


				initAutocomplete(g_data[0],g_data[1]);
...
function initAutocomplete(in_lat,in_lng) {
...
//		center: {lat: in_lat, lng: in_lng},
	...
	map.setCenter( new google.maps.LatLng( in_lat, in_lng ) ) ;
