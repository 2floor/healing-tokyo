	...
	function page_init(){
		...
		$('.calender').each(function() {
			...
			$(id + ' input').bind('change', function() {
				...
				//2週間カレンダー全て非表示へ
				$('.two_w_cal').hide();
				$('.two_w_cal').removeClass('l_date');
				$('.two_w_cal').removeClass('f_date');
				...
				for (var i = 0; i < 14; i++) {
					...
					if(i == 0){
						$('#'+plus_day).addClass('f_date');
					}else if(i == 13){
						$('#'+plus_day).addClass('l_date');
					}
...
//	 		showCurrentAtPos : 1,
		...
		$('#next').bind('click', function() {
			...
			var l_date = moment($('.l_date').attr('date'));
			console.log(l_date);
			l_date = l_date.add(1, "day").format("YYYY-MM-DD");
			console.log(l_date);
			$('.two_w_cal').removeClass('l_date');
			$('.two_w_cal').removeClass('f_date');
			...
			//月カレンダーで指定された日付から2週間分を表示
			for (var i = 0; i < 14; i++) {
				var date = moment(l_date);
				var plus_day = date.add(i, "day").format("YYYY-MM-DD");
				$('.two_w_cal[date="'+plus_day + '"]').show();

				if(i == 0){
					$('.two_w_cal[date="'+plus_day + '"]').addClass('f_date');
				}else if(i == 13){
					$('.two_w_cal[date="'+plus_day + '"]').addClass('l_date');
				}
				if(!$('.two_w_cal[date="'+plus_day + '"]').size()){
					$('#next').hide();
				}
			}
		});
		...
		$('#prev').bind('click', function() {
			...
			var f_date = moment($('.f_date').attr('date'));
			f_date = f_date.add(-1, "day").format("YYYY-MM-DD");
			$('.two_w_cal').removeClass('l_date');
			$('.two_w_cal').removeClass('f_date');
			...
			for (var i = 0; i < 14; i++) {
				var j =  i*-1;
				var date = moment(f_date);
				var plus_day = date.add(j, "day").format("YYYY-MM-DD");
				$('.two_w_cal[date="'+plus_day + '"]').show();
				...
				if($('.two_w_cal[date="'+plus_day + '"]').hasClass('first')){
					$('#prev').hide();
				}
			}
		});
