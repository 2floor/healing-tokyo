...
$(function() {
	...
	function get_form_prams (method, now_page_num, get_next_disp_page, page_disp_cnt) {
		...
		fd.append('staff_title_array', file_name.staff_title_array);
...
//				$('#conf_text').html('以下の項目はすべて<span class="registTit2">※必須項目</span>です。');
	...
	function page_init(){
		...
		// メソッド定義
		var form_datas = get_form_prams('init_store_detail_staff');
	...
	function call_ajax_detail_init(post_data, flg){
		ajax.get(post_data).done(function(result) {
			...
			if (result.data.status) {
				...
				var bool3 = result.data.etc1 != null && result.data.etc1 != "";
						...
						if(key == 'chef_staff'){
							$.each(elem, function(key_2, elem_2) {
								if(elem_2 != null && elem_2 != ''){
									...
									$('#img_area' + cnt).append('<img class="storeEditImg2 " src="../upload_files/store/store_id'+result.data.store_basic_id+'/store_detail_img/chef_staff/'+elem_2+'">')
								}
								cnt++;
							});
						}
				...
				if(bool3){
					//店内写真コメント
					staff_title_array = JSON.parse(result.data.etc1.replace(/\u2028|\u2029/g, ''));
					$.each(staff_title_array, function(key, elem) {
						cnt = 1;
						if(key == 'title'){
							$.each(elem, function(key_2, elem_2) {
								if(elem_2 != null && elem_2 != ''){
									$('.store_detail_title_' + cnt).val(elem_2)
								}
								cnt++;
							});
						}else if(key == 'job'){
							$.each(elem, function(key_2, elem_2) {
								if(elem_2 != null && elem_2 != ''){
									$('.store_job_title_' + cnt).val(elem_2)
								}
								cnt++;
							});
						}
	...
	function edit_exection(){
		//method定義
		var entry_datas = get_form_prams('edit_store_detail_staff', null, null, null);
		//ajax呼び出し
	...
	function get_file_name(){
		...
		img_name_array.chef_staff = [];

		img_comment_array.chef_staff = [];

		//t_store_basic => etc1使用
		var staff_title_array = {
				'title': [],
				'job': [],
		};
		...
		$('input').each(function(){
			if($(this).attr('type') == 'file'){
				...
				if(this.files[0] != undefined){
					img_name_array.chef_staff[cnt] = this.files[0].name;
				}else{
					img_name_array.chef_staff[cnt] = $('span.img_name_' + cnt).text();
				}
			}
		});

		//タイトル取得
		$('.title_area').each(function(i){
			staff_title_array.title[i] = $(this).val();
		});

		//役職取得
		$('.job_area').each(function(i){
			staff_title_array.job[i] = $(this).val();
		});

		//コメント取得
		$('.comment_area').each(function(i){
			img_comment_array.chef_staff[i] = $(this).val();
		});

		//json形式に変換
		img_name_array = JSON.stringify(img_name_array);
		img_comment_array = JSON.stringify(img_comment_array);
		staff_title_array = JSON.stringify(staff_title_array);

		var return_array;
		return return_array = {
				'img_name_array':img_name_array,
				'img_comment_array':img_comment_array,
				'staff_title_array':staff_title_array,
		};
	}
