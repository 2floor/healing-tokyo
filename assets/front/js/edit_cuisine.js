...
//				$('#conf_text').html('以下の項目はすべて<span class="registTit2">※必須項目</span>です。');
	...
	function call_ajax_detail_init(post_data, flg){
		ajax.get(post_data).done(function(result) {
			...
			if (result.data.status) {
				...
				if(bool1){console.log(result.data)
						...
						if(key == 'cuisine'){
							$.each(elem, function(key_2, elem_2) {
								if(elem_2 != null && elem_2 != ''){
									...
									$('#img_area' + cnt).append('<img class="storeEditImg2 " src="../upload_files/store/store_id'+result.data.store_basic_id+'/store_detail_img/cuisine/'+elem_2+'">')
								}
								cnt++;
							});
						}
	...
	function get_file_name(){
		...
		img_name_array.cuisine = [];

		img_comment_array.cuisine = [];
		...
		$('input').each(function(i){
			if($(this).attr('type') == 'file'){
				...
				if(this.files[0] != undefined){
					img_name_array.cuisine[cnt] = this.files[0].name;
				}else{
					img_name_array.cuisine[cnt] = $('span.img_name_' + cnt).text();
				}
			}
		});
		...
		$('.comment_area').each(function(i){
			img_comment_array.cuisine[i] = ($(this).val());
		});
