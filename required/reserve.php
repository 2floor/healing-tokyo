...
$jis_common_logic->login_check("rsv", $_GET['trid'] );
if($_SESSION['jis']['ty'] == "1"){

}else{
	header("Location: ../login.php?er=rsv&r".$_GET['trid']);
	exit();
}
...
if(strpos($tour['tour']['payment_way'], "0") !== false ){
	$payment_way .= '<label class="formRBox radioStyle"><input type="radio" name="payment_way" value="1" class="validate" style="width: 15px; height: auto;" checked="checked"><span>Payable cash</span></label>';
}
if(strpos($tour['tour']['payment_way'], "2") !== false ){
	$payment_way .= '<label class="formRBox radioStyle"><input type="radio" name="payment_way" value="3" class="validate" style="width: 15px; height: auto;"><span>Credit card('.$tour['tour']['card_choice'].')</span></span></label>';
}

if(strpos($tour['tour']['payment_way'], "1") !== false ){
	$payment_way .= '<label class="formRBox radioStyle"><input type="radio" name="payment_way" value="2" class="validate" style="width: 15px; height: auto;"><span>Advance payment required by opperating campany</span></span></label>';
}
				...
				<div class="container760" id="inputFormArea">
						...
						<div class="storeEditItem">
							<div class="storeEditRow">
								...
								<div class="storeEditRow">
									...
									<div class="storeEditForm">
										<span>¥ <?php print number_format($descPrioce)?>　per person</span>
									</div>
								</div>
								<div class="storeEditRow">
									<div class="storeEditCate">Chilren’s fare</div>
									<div class="storeEditForm">
										<span>¥ <?php print number_format($childDescPrioce)?>　per person</span>
									</div>
								</div>
								<div class="storeEditRow">
					...
					<section >
						<div class="agreementCheckBoxArea">
							<label class="noteTextArea checkboxStyle"><input type="checkbox" name="available_card" value="0" class="validate" style="width: 15px; height: auto;"><span>I agree with the above</span></label>
						</div>
					</section>
									...
									<div class="storeEditForm required_form">
										<p>Reason for choosing?</p>
											<select name="question_answer" id="question_answer" class="formTxt1 validate required">
												<option value="0">Activity is interesting.</option>
												...
												<option value="4">Experience in the past.</option>
											</select>
									</div>
								</div>
					<section>
