	<div class="loading">
		<div class="loading_icon"></div>
	</div>
	<script>
		var resizefunc = [];
	</script>
	<!-- jQuery  -->
	<script src="../assets/admin/js/common/jquery.min.js"></script>
	<script src="../assets/admin/js/common/jquery-ui.min.js"></script>
	<script src="../assets/admin/js/common/bootstrap.min.js"></script>
	<script src="../assets/admin/js/common/detect.js"></script>
	<script src="../assets/admin/js/common/fastclick.js"></script>
	<script src="../assets/admin/js/common/jquery.slimscroll.js"></script>
	<script src="../assets/admin/js/common/jquery.blockUI.js"></script>
	<script src="../assets/admin/js/common/waves.js"></script>
	<script src="../assets/admin/js/common/jquery.nicescroll.js"></script>
	<script src="../assets/admin/js/common/jquery.scrollTo.min.js"></script>
	<!-- Plugins Js -->
	<script src="../assets/admin/plugins/switchery/switchery.min.js"></script>
	<script src="../assets/admin/plugins/bootstrap-tagsinput/dist/bootstrap-tagsinput.min.js"></script>
	<script type="text/javascript" src="../assets/admin/plugins/multiselect/js/jquery.multi-select.js"></script>
	<script type="text/javascript" src="../assets/admin/plugins/jquery-quicksearch/jquery.quicksearch.js"></script>
	<script src="../assets/admin/plugins/select2/dist/js/select2.min.js" type="text/javascript"></script>
	<script src="../assets/admin/plugins/bootstrap-touchspin/dist/jquery.bootstrap-touchspin.min.js" type="text/javascript"></script>
	<script src="../assets/admin/plugins/bootstrap-inputmask/bootstrap-inputmask.min.js" type="text/javascript"></script>
	<script src="../assets/admin/plugins/moment/moment.js"></script>
	<script src="../assets/admin/plugins/timepicker/bootstrap-timepicker.min.js"></script>
	<script src="../assets/admin/plugins/mjolnic-bootstrap-colorpicker/dist/js/bootstrap-colorpicker.min.js"></script>
	<script src="../assets/admin/plugins/bootstrap-datepicker/dist/js/bootstrap-datepicker.min.js"></script>
	<script src="../assets/admin/plugins/bootstrap-daterangepicker/daterangepicker.js"></script>
	<script src="../assets/admin/plugins/bootstrap-maxlength/bootstrap-maxlength.min.js" type="text/javascript"></script>
	<script src="../assets/admin/plugins/bootstrap-sweetalert/sweet-alert.min.js"></script>
	<script src="../assets/admin/plugins/custombox/dist/custombox.min.js"></script>
	<script src="../assets/admin/plugins/custombox/dist/legacy.min.js"></script>
	<!-- App js -->
	<script src="../assets/admin/js/common/jquery.core.js"></script>
	<script src="../assets/admin/js/common/jquery.app.js"></script>
	<script src="../assets/admin/js/common/plural_file_upload.js"></script>
	<script src="../assets/admin/js/common/validate.js"></script>
	<script src="../assets/admin/js/common.js"></script>

