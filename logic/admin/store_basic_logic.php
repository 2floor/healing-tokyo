...
// 			}
			...
			$return_html .= "
					<tr " . $back_color_html . ">
						...
						<td>TEL : " . '+' . $row['country_phone'] . ' ' . $row['tel'] . "<br>MAIL: " . $row['mail'] . "<br>FAX : " . $row['fax'] . "</td>
						<td>" . $auth . "</td>
						<td>" . $create_at . "</td>
						<td>" . $update_at . "</td>
						<td>
...
この度は、healing-tokyo事業者会員にご登録頂き、誠にありがとうございます。
...
また、当サイトは顧客からの予約を即時完結し、予約完了メールまで自動配信するシステムとなっています。
そのため、空き人数管理（ブロック数管理）は大変重要な事項ですので、こちらも必ず先にお読み下さい。
空き人数管理（ブロック数管理）に関して https://healing-tokyo.com/manual/Block_management_manual.pdf

会員情報編集は https://healing-tokyo.com/manual/Company_Information_Management_Registration_Manual.pdf をご参考に入力をお願い致します。
...
';
		...
		$header .= "From:" .mb_encode_mimeheader("Japan International Services") ."<"."no-reply@healing-tokyo.com".">\n";
		$header .= "Sender: " . "healing-tokyo" ."<"."no-reply@healing-tokyo.com".">\n";
		$header .= "Reply-To: " .mb_encode_mimeheader("Japan International Services") ."<"."no-reply@healing-tokyo.com".">\n";
		$header .= "Organization: " . "no-reply@healing-tokyo.com" . " \n";
		$header .= "X-Sender: " . "no-reply@healing-tokyo.com" . " \n";
		$header .= "X-Priority: 3 \n";


		mb_send_mail($store[0]['mail'], "【healing-tokyo】事業者様会員登録完了のお知らせ", $body, $header);
