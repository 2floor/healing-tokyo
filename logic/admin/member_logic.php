		...
		for($i = 0; $i < count ( $result_member ); $i ++) {
			...
			if (strpos ( $row ['image'], ',' ) !== false && ($row ['image'] != null && $row ['image'] != '')) {
				...
				for($n = 0; $n < count ( $nmage_list ); $n ++) {
					$img_tag_html .= '<img src="../upload_files/member/' . $nmage_list [$n] . '" style="height:50px">';
				}
			} else if ($row ['image'] != null && $row ['image'] != '') {
				$img_tag_html = '<img src="../upload_files/member/' . $row ['image'] . '" style="height:50px">';
			}

			//動画
			if ($row['movie'] != null && $row['movie'] != ""){
				$movie = '<a  href="#modal" class="check_movie" member_id="'. $row['member_id'] .'">有り</a>';
			}else{
				$movie = '無し';
			}
			...
			$return_html .= "
					<tr " . $back_color_html . ">
						...
						<td>" . $row['member_id'] . "</td>
						<td>" . $row['name'] . "<br>" . $row['nationality'] . "</td>
						<td>TEL : ".'+' . $row['country_phone'] .' '. $row['tel'] . "<br>MAIL: " . $row['mail'] . "</td>
						<td>" . $create_at . "</td>
						<td>" . $update_at . "</td>
						<td>
							$edit_html_a
