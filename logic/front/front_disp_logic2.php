...
class front_disp_logic {
    ...
    public function get_my_tour(){
        ...
        foreach ((array)$tour as $row) {
	       	...
	       	$offer = $this->common_logic->select_logic("select * from t_eng_offer where tour_id = ? order by create_at", array($row['tour_id']));
	       	$offer_str = '';
	       	if($offer != null && $offer != ''){
	       		if($offer[0]['status'] == '1'){
...
// 		"));
//         print"</pre>";
//         var_dump($where_array_m);
//         foreach ((array)$tour as $tr) {
//         	var_dump($tr['start_date'] . '～'. $tr['end_date'] . ":".$tr['max_number_of_people'] );
//         }
    ...
    public function get_tour_detail($tour_id) {
        ...
        $result['payment_way'] = str_replace(array("0", "1", "2"), array("Cash on site", "Immediate credit card payment", "Credit card on site"), $tour['tour']['payment_way'] );
							...
							<div class="planCalBox2">
								...
								<div class="planCalRemain2">
									...
									<div class="planDetailClose">
										'.$result['tour_base']['title'].'<br>￥'.number_format($price).'
									</div>
								</div>
							</div>';
    ...
    public function get_reserve_list_for_top($tour_id = null){
        ...
        $rsv = $this->common_logic->select_logic("
				SELECT `reservation_id`, `tour_relation_id`, `tour_id`, `title`, `start_date`, `start_time`,`order_id`,`cancel_flg`,`payment_way`, (`men_num` + `women_num` + `children_num` ) AS `total_num`, `total_add_tax`, `come_date`, `m`.`name`, `m`.`tel`
				FROM `t_reservation`
				INNER JOIN (
...
//         	}
            ...
            $html .= '
				<div class="mypageReservBox " '.$cancel_color.' dt="'.date('Y-m-d', strtotime($row['come_date'])).'">
					<div class="mypageReservStoreRow">
						<div class="mypageReservDate">
...
';
    ...
    public function get_index_html(){
            ...
            $arr_html .= '<div>
                <div class="card-wrapper flip-card">
                    ...
                    <div class="card-box">
						...
						<p class="mbr-card-price display-5 b">￥'.number_format($price).' / Day</p>
						</a>
                    </div>
                </div>
            </div>';
        ...
        foreach ($category_array as $category_id => $str) {
        	...
        	$cate_tour_data = $this->get_cate_tour_data_ar($cate_ar, 4);
...
// 		return $this->common_logic->select_logic("select `tour_id`, `create_at` from t_tour INNER JOIN (select `tour_id` from t_tour_relation where del_flg = 0 and public_flg = 0 and CURDATE() between `start_date` and `end_date` ) as`rel` USING(tour_id)  where del_flg = 0 and public_flg = 0 ".$where['where']." order by create_at DESC limit ? ", $whereP);
		return $this->common_logic->select_logic("select `tour_id`, `create_at` from t_tour INNER JOIN (select `tour_id` from t_tour_relation where del_flg = 0 and public_flg = 0 and CURDATE() <= `end_date` ) as`rel` USING(tour_id)  where del_flg = 0 and public_flg = 0 ".$where['where']." order by create_at DESC limit ? ", $whereP);
    }
...
// 		return $this->common_logic->select_logic("select `tour_id`, `rsv_num` from t_tour INNER JOIN (select `tour_id`, `rsv_num` from t_tour_relation INNER JOIN ( select count(reservation_id) as rsv_num, tour_relation_id from t_reservation where cancel_flg = 0 and del_flg = 0 group by tour_relation_id ) as`rsv` USING(tour_relation_id) where del_flg = 0 and public_flg = 0 and CURDATE() <= `end_date`) as`rel` USING(tour_id)  where del_flg = 0 and public_flg = 0 ".$where['where']." order by rsv_num DESC limit ? ", $whereP);

        return $this->common_logic->select_logic("select count(`reservation_id`) as `rsv_num`, `tour_id`, `store_basic_id` from t_reservation INNER JOIN ( SELECT `tour_id`, `tour_relation_id` from t_tour_relation where del_flg = 0 and public_flg = 0 ) as `tr` USING(`tour_relation_id`) INNER JOIN ( SELECT `store_basic_id` from t_store_basic where del_flg = 0 and auth_flg = 1 ) as `st` USING(`store_basic_id`)  INNER JOIN ( SELECT `tour_id` from t_tour where del_flg = 0 and public_flg = 0 ) as `t` USING(`tour_id`) where cancel_flg = 0 and del_flg = 0 group by `tour_id` order by `rsv_num` DESC LIMIT ? ", $whereP);
...
// 		return $this->common_logic->select_logic("select `tour_id`, `create_at` from t_tour INNER JOIN (select `tour_id` from t_tour_relation where del_flg = 0 and public_flg = 0 and CURDATE() between `start_date` and `end_date` ) as`rel` USING(tour_id)  where del_flg = 0 and public_flg = 0 and find_in_set(?, `category`) order by create_at DESC limit ? ", array($category_id, $limit));
		return $this->common_logic->select_logic("select `tour_id`, `create_at` from t_tour INNER JOIN (select `tour_id` from t_tour_relation where del_flg = 0 and public_flg = 0 and CURDATE() <= `end_date` ) as`rel` USING(tour_id)  where del_flg = 0 and public_flg = 0 and find_in_set(?, `category`) order by create_at DESC limit ? ", array($category_id, $limit));
    }
    public function get_cate_tour_data_ar($category_id_ar, $limit){
    	$where_cate_ar = array();
    	$where_param = array();
    	...
    	return $this->common_logic->select_logic("select `tour_id`, `create_at` from t_tour INNER JOIN (select `tour_id` from t_tour_relation where del_flg = 0 and public_flg = 0 and CURDATE() <= `end_date` ) as`rel` USING(tour_id)  where del_flg = 0 and public_flg = 0  AND ( ".implode(" OR ", $where_cate_ar)." ) order by create_at DESC limit ? ", $where_param);
    }
