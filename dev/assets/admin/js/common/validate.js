...
function validate_start(){
	$(':text, :password, [type="tel"], [type="email"], textarea, select').off('.validate');
	$(':text, :password, [type="tel"], [type="email"], textarea, select').on({
		...
		if(type != 'radio' && type != 'checkbox' && type != 'file' && !check_elem.hasClass('couple') ){
			...
			if(check_elem.hasClass('validate')){
				...
				if(type == 'email'){
					...
					}else if(check_elem.val() && check_elem.val().match(/[^A-Za-z0-9]+/)){
				...
				if(check_elem.hasClass('password')){
					...
					if(check_elem.val() != null && check_elem.val() != '' && ((check_elem.val().length < 8 || check_elem.val().length > 20) && !check_elem.val().match(/[^a-zA-Z]/) || !check_elem.val().match(/[^0-9]/))){
							re_bool = disp_error_msg(check_elem, '半角英字と半角数字を含む8～20文字を設定して下さい');
					}
					if( check_elem.val() != null && check_elem.val().match(/[ぁ-んａ-ｚＡ-Ｚ]/) != null){
						re_bool = disp_error_msg(check_elem, '半角英字と半角数字を含む8～20文字を設定して下さい');
					}
				}
...
function validate_all(){
	...
	$(':text, :password, :radio, :file, :checkbox, [type="password"], [type="tel"], [type="email"], textarea, select').each(function(){
		if($(this).hasClass('validate')){
			realtime_validate($(this), true);
		}
	});
