	...
	function authBind(){
		...
		$('.auth').on('click',function(){
			...
			swal({
				...
				if (isConfirm) {
					// 呼び出し前method定義
					var form_data = append_form_prams('auth', 'frm',
							null, null, null, null);
		...
		//非公開処理
		$('.auth_inna').off();
		$('.auth_inna').on('click',function(){
			var id = $(this).attr('value');

			swal({
				title : "未認証にする",
				text : '管理ID' + id + "を未認証にします。よろしいですか？",
				type : "warning",
				showCancelButton : true,
				confirmButtonClass : 'btn-warning',
				confirmButtonText : "未認証にする",
				cancelButtonText : 'キャンセル',
				closeOnConfirm : false,
				closeOnCancel : false
			}, function(isConfirm) {
				if (isConfirm) {
					// 呼び出し前method定義
					var form_data = append_form_prams('auth_inna', 'frm',
							null, null, null, null);
					form_data.append('id', id);
