...
//				$('#conf_text').html('以下の項目はすべて<span class="registTit2">※必須項目</span>です。');
						...
						if(key == 'exterior'){
							$.each(elem, function(key_2, elem_2) {
								if(elem_2 != null && elem_2 != ''){
									...
									var img_cnt = cnt+1;
									$('#img_area' + cnt).append('<img class="storeEditImg2 " src="../upload_files/store/store_id'+result.data.store_basic_id+'/store_detail_img/exterior/'+elem_2+'">')
								}
								cnt++;
							});
						}
	...
	function get_file_name(){
		...
		$('input').each(function(i){
			if($(this).attr('type') == 'file'){
				...
				if($(this)[0].files[0] != undefined){
					img_name_array.exterior[cnt] = $(this)[0].files[0].name;
				}else{
					img_name_array.exterior[cnt] = $('span.img_name_' + cnt).text();
				}
			}
		});
		...
		$('.comment_area').each(function(i){
			img_comment_array.exterior[i] = $(this).val();
		});
