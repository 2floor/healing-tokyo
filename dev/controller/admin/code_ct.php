<?php
session_start ();
// header('Content-Type: applicodeion/json');

require_once __DIR__ .  '/../../logic/common/common_logic.php';
require_once __DIR__ .  '/../../logic/common/common_string_logic.php';
require_once __DIR__ .  '/../../logic/admin/code_logic.php';
...
if ($data ['status']) {
	...
	// コントローラー呼び出し
	$data = $code_ct->main_control ( $_POST );
} else {
	// パラメータに不正があった場合
	// AJAX返却用データ成型
	$data = array (
...
class code_ct {
	...
	public function __construct() {
		// 管理画面ユーザーロジックインスタンス
		$this->code_logic = new code_logic ();
	...
	public function main_control($post) {
		...
		if ($post ['method'] == 'init') {
			// 初期処理　HTML生成処理呼び出し
			$data = $code_ct->create_data_list ( $post );
		} else if ($post ['method'] == 'entry') {
			// 新規登録処理
			$data = $code_ct->entry_new_data ( $post );
		} else if ($post ['method'] == 'edit_init') {
			// 編集初期処理
			$data = $code_ct->get_detail ( $post ['edit_del_id'] );
		} else if ($post ['method'] == 'edit') {
			// 編集更新処理
			$data = $code_ct->update_detail ( $post );
		} else if ($post ['method'] == 'delete') {
			// 削除処理
			$data = $code_ct->delete ( $post ['id'] );
		} else if ($post ['method'] == 'recovery') {
			// 有効化処理
			$data = $code_ct->recovery ( $post ['id'] );
		} else if ($post ['method'] == 'private') {
			// 非公開化処理
			$data = $code_ct->private_func ( $post ['id'] );
		} else if ($post ['method'] == 'release') {
			// 公開化処理
			$data = $code_ct->release ( $post ['id'] );
		}
	...
	private function create_data_list($post) {
		...
		$list_html = $this->code_logic->create_data_list ( array (
	...
	private function entry_new_data($post) {
		...
		// 登録ロジック呼び出し
		$this->code_logic->entry_new_data ( array (
				$post['etc1'],
				$post['etc2'],
				$post['etc3'],
				$post['etc4'],
	 ...
	 */
	private function get_detail($code_id) {
		$reult_detail = $this->code_logic->get_detail ( $code_id );

		// AJAX返却用データ成型
		$data = array (
				'status' => true,
				'code' => $reult_detail ['code'],
				'description1' => $reult_detail ['description1'],
				'description2' => $reult_detail ['description2'],
				'description3' => $reult_detail ['description3'],
			);
	...
	private function update_detail($post) {
		...
		// 編集ロジック呼び出し
		$this->code_logic->update_detail ( array (
				$post['description1'],
				$post['description2'],
				$post['description3'],
				$post ['edit_del_id']
	...
	public function recovery($id) {
		// 更新ロジック呼び出し
		$this->code_logic->recoveryl_func ( $id );
	...
	public function private_func($id) {
		// 更新ロジック呼び出し
		$this->code_logic->private_func ( $id );
	...
	public function release($id) {
		// 更新ロジック呼び出し
		$this->code_logic->release_func ( $id );
