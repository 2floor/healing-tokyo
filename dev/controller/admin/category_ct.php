...
class category_ct {
	...
	private function create_data_list($post) {
		...
		$list_html = $this->category_logic->create_data_list ( array (
	...
	private function entry_new_data($post) {
		...
		$hi = $this->common_logic->select_logic("select * from t_category where category_id = ? ", array($post['dear_id']));

		// 登録ロジック呼び出し
		$this->category_logic->entry_new_data ( array (
				$post['dear_id'],
				$hi[0]['hierarchy'] + 1,
				$post['category'],
				$post['category_eng'],
	 ...
	 */
	private function get_detail($category_id) {
		$reult_detail = $this->category_logic->get_detail ( $category_id );
		...
		$data = array (
				...
				'category_eng' => $reult_detail['category_eng'],
	...
	private function update_detail($post) {
		...
		// 編集ロジック呼び出し
		$this->category_logic->update_detail ( array (
				$post['dear_id'],
				$hi[0]['hierarchy'] + 1,
				$post['category'],
				$post['category_eng'],
	...
	public function recovery($id) {
		// 更新ロジック呼び出し
		$this->category_logic->recoveryl_func ( $id );
	...
	public function delete($id) {
		// 更新ロジック呼び出し
		$this->category_logic->del_func ( $id );
	...
	public function private_func($id) {
		// 更新ロジック呼び出し
		$this->category_logic->private_func ( $id );
	...
	public function release($id) {
		// 更新ロジック呼び出し
		$this->category_logic->release_func ( $id );
