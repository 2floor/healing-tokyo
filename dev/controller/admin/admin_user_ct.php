...
class admin_user_ct {
	...
	public function main_control($post) {
		...
		if ($post ['method'] == 'init') {
			// 初期処理　HTML生成処理呼び出し
			$data = $admin_user_ct->create_admin_user_list_html ();
		} else if ($post ['method'] == 'edit_init') {
			// 編集初期処理
			$data = $admin_user_ct->get_admin_user_detail_html ( $post ['edit_del_id'] );
		} else if ($post ['method'] == 'edit') {
			// 編集更新処理
			$data = $admin_user_ct->update_admin_user_detail ( $post );
		} else if ($post ['method'] == 'del') {
			// 削除処理
			$data = $admin_user_ct->del_admin_user ( $post ['del_id'] );
		} else if ($post ['method'] == 'entry') {
			// 新規ユーザー登録処理
			$data = $admin_user_ct->entry_admin_user ( $post );
		} else if ($post ['method'] == 'recovery') {
			// ユーザー有効化処理
			$data = $admin_user_ct->recovery ( $post ['recovery_id'] );
		}
