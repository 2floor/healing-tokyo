<?php
session_start ();
// header('Content-Type: applireservationion/json');

require_once __DIR__ .  '/../../logic/common/common_logic.php';
require_once __DIR__ .  '/../../logic/common/common_string_logic.php';
require_once __DIR__ .  '/../../logic/admin/reservation_logic.php';
...
if ($data ['status']) {
	...
	// インスタンス生成
	$reservation_ct = new reservation_ct ();

	// コントローラー呼び出し
	$data = $reservation_ct->main_control ( $_POST );
} else {
	// パラメータに不正があった場合
	// AJAX返却用データ成型
	$data = array (
...
class reservation_ct {
	...
	public function __construct() {
		// 管理画面ユーザーロジックインスタンス
		$this->reservation_logic = new reservation_logic ();
	...
	public function main_control($post) {
		...
		if ($post ['method'] == 'init') {
			// 初期処理　HTML生成処理呼び出し
			$data = $reservation_ct->create_data_list ( $post );
		} else if ($post ['method'] == 'entry') {
			// 新規登録処理
			$data = $reservation_ct->entry_new_data ( $post );
		} else if ($post ['method'] == 'edit_init') {
			// 編集初期処理
			$data = $reservation_ct->get_detail ( $post ['edit_del_id'] );
		} else if ($post ['method'] == 'edit') {
			// 編集更新処理
			$data = $reservation_ct->update_detail ( $post );
		} else if ($post ['method'] == 'delete') {
			// 削除処理
			$data = $reservation_ct->delete ( $post ['id'] );
		} else if ($post ['method'] == 'recovery') {
			// 有効化処理
			$data = $reservation_ct->recovery ( $post ['id'] );
		} else if ($post ['method'] == 'private') {
			// 非公開化処理
			$data = $reservation_ct->private_func ( $post ['id'] );
		} else if ($post ['method'] == 'release') {
			// 公開化処理
			$data = $reservation_ct->release ( $post ['id'] );
		}
	...
	private function create_data_list($post) {
		...
		$list_html = $this->reservation_logic->create_data_list ( array (
	...
	private function entry_new_data($post) {
		...
		// 登録ロジック呼び出し
		$this->reservation_logic->entry_new_data ( array (
				$post['etc1'],
				$post['etc2'],
				$post['etc3'],
				$post['etc4'],
	 ...
	 */
	private function get_detail($reservation_id) {
		$reult_detail = $this->reservation_logic->get_detail ( $reservation_id );
	...
	private function update_detail($post) {
		...
		// 編集ロジック呼び出し
		$this->reservation_logic->update_detail ( array (
				$post['etc1'],
				$post['etc2'],
				$post['etc3'],
				$post['etc4'],
	...
	public function recovery($id) {
		// 更新ロジック呼び出し
		$this->reservation_logic->recoveryl_func ( $id );
	...
	public function delete($id) {
		// 更新ロジック呼び出し
		$this->reservation_logic->del_func ( $id );
	...
	public function private_func($id) {
		// 更新ロジック呼び出し
		$this->reservation_logic->private_func ( $id );
	...
	public function release($id) {
		// 更新ロジック呼び出し
		$this->reservation_logic->release_func ( $id );
