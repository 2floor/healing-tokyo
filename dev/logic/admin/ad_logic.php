...
// 		$sqlAdd['where'] = $add . " hierarchy = 0 ";
		...
		for($i = 0; $i < count ($result_ad  ); $i ++) {
			...
			if (strpos ( $row ['image'], ',' ) !== false && ($row ['image'] != null && $row ['image'] != '')) {
				...
				$img_tag_html = '<img src="../upload_files/ad/' . $row ['image'] . '" style="height:50px">';
			}
			...
			$return_html .= "
					<tr " . $back_color_html . ">
						...
						<td>" . $row['ad_id'] . "</td>
						<td>" . $row['ad'] . "</td>
						<td><img src='../../upload_files/ad/" . $row['thumbnail'] . "' style='height:50px'></td>
						<td><a href='" . $row['ad_eng'] . "' style='height:50px' target='_blank'>URL</a></td>
						<td>" . $create_at . "</td>
						<td>" . $update_at . "</td>
						<td>
							$edit_html_a
...
";
	...
	public function conv($row, $hi, $cnt,$back_color) {
		...
		if ($row ['del_flg'] == 1) {
			$del_color = "color:#d3d3d3";
			$del_html = "削除";
			if($row['hierarchy'] != 0)$edit_html_a .= "<a herf='javascript:void(0);' class='recovery clr2' name='recovery_" . $row ['ad_id'] . "' value='" . $row ['ad_id'] . "' ><i class=\"fa fa-undo\" aria-hidden=\"true\"></i></a><br>";
		} else {
			if($row['hierarchy'] != 0)$edit_html_a .= "<a herf='javascript:void(0);' class='del clr2' name='del_" . $row ['ad_id'] . "' value='" . $row ['ad_id'] . "'><i class=\"fa fa-trash\" aria-hidden=\"true\"></i></a><br>";
		}
