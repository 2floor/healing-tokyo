...
class common_logic {
	...
	public function google_oauth_logic($value) {
...
// 		$header = "From: " . $header;
	 ...
	 * @return 結果(boolean)
	 */
	public function insert_logic($tb_name, $param_array, $return_id = null) {
		...
		// query実行
		$db = new db_access ();
		$result = $db->insert_update_executed ( $query, $param_array, $return_id );
		return $result;
	}
	...
	public function front_unit_file_upload($file, $upload_path){
		...
		$this->chkDirectory(explode('/', $upload_path));

		if (move_uploaded_file ( $fileTmpLoc, $upload_path . $fileName )) {
			if(strpos($fileName,'.jpg') !== false && strpos($fileName,'.JPG') !== false && strpos($fileName,'.png') !== false && strpos($fileName,'.PNG') !== false){
				$this->imageRotation($upload_path . $fileName);
			}
	...
	public function unit_file_upload($file, $upload_path){
		$fn_a = explode(".", $file ["name"]);
		$ext = array_pop($fn_a);
		$fileName = ceil(microtime(true)) . '-' .$this->getRandomString(5) . "." . mb_strtolower($ext); // $file ["name"]; // The file name
		$fileTmpLoc = $file ["tmp_name"]; // File in the PHP tmp folder
		$fileType = $file ["type"]; // The type of file it is
		$fileSize = $file ["size"]; // File size in bytes
		$fileErrorMsg = $file ["error"]; // 0 for false... and 1 for true
		...
		$this->chkDirectory(explode('/', $upload_path));
		if (move_uploaded_file ( $fileTmpLoc, $upload_path . $fileName )) {
			if(strpos($fileName,'.jpeg') !== false || strpos($fileName,'.JPEG') !== false || strpos($fileName,'.jpg') !== false || strpos($fileName,'.JPG') !== false || strpos($fileName,'.png') !== false || strpos($fileName,'.PNG') !== false){
				$this->imageRotation($upload_path . $fileName);
				$this->image_compression($upload_path . $fileName);
			}
			$ret_array = implode(',', array (
					$upload_path . $fileName,
					$fileName
	 ...
	 * 画像回転
	 */
	public function imageRotation($file_name){
		// 存在チェック
		...
		$exif = @exif_read_data($file_name, 'EXIF');
		if($exif['Orientation'] == null || $exif['Orientation'] == '') return false;
	 ...
	 * 画像圧縮
	 */
	function image_compression($image_url, $h = 1024, $w = 1024){
		list($originalWidth, $originalHeight, $type) = getimagesize($image_url);
		switch ($type) {
			case IMAGETYPE_JPEG:
		...
		list($width, $height) = $this->get_contain_size($originalWidth, $originalHeight, $w, $h);

		$canvas = imagecreatetruecolor($width, $height);
		imagecopyresampled($canvas, $source, 0, 0, 0, 0, $width, $height, $originalWidth, $originalHeight);
		imagejpeg($canvas, $image_url);
		imagedestroy($source);
		imagedestroy($canvas);
	}

	public function get_contain_size($width, $height, $containerWidth, $containerHeight)	 {
		$ratio = $width / $height;
...
// 	require_once __DIR__ .  '/model/m_area_pref_model.php';
	 ...
	 */
	function create_table_dump($tb_name){
		...
		//パス
		$filename_path = __DIR__ . '/../../NoUploadFiles/table_dump/';
	...
	function create_where($search_select_no_obj, $whereInit = null , $whereParamInit = array()){
		...
		if($search_select['exception'] != null){
			$value_array = array();
