...
$data = $front_disp_logic->get_review_rank_data(10);
$i = 0;
foreach ((array)$data as $d){
	$row = $jis_common_logic->get_store_basic($d['store_basic_id']);
// 	$row = $front_disp_logic->get_tour_detail_data($d['tour_id']);

	++$i;
	$rank = (strlen($i) == 1) ? "0".$i: $i;
	$img = explode(",", $row['img']);
// 	$store = $jis_common_logic->get_store_basic($row['store_basic_id']);
	...
	$html .= '<div class="planBox1 mT40">
							...
							<div class="searchResultBoxPlan">
								<h4 class="rankingStoreName">
									<a href="./plan.php?tid='.$row['tour_id'].'&sbid='.$row['store_basic_id'].'">'.$row['company_name_eng'].'</a>
									<span class="rankingNameArea"></span>
								</h4>
								<div class="searchResultBoxPlanImg">
									<img src="../upload_files/store_basic/'.$img[0].'" alt="'.$row['title'].'">
								</div>
		...
		<div id="wrap1080">
							...
							<div class="">
							    <div class="sp-slides">
							        ...
							        <div class="sp-slide sliderBorBtm">
							            <img class="sp-image" src="../assets/front/img/tit_review_ranking.jpg"/>
							        </div>
								 </div>
							  </div>
						</section>
			...
			<div class="container1080 cf">
				<div class="container760">
					...
					<section>
						<h3 class="titleUnderline">Word of mouth ranking</h3>
