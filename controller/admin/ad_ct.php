...
require_once __DIR__ .  '/../../logic/admin/ad_logic.php';
...
class ad_ct {
	...
	public function main_control($post) {
		...
		if ($post ['method'] == 'init') {
			// 初期処理　HTML生成処理呼び出し
			$data = $ad_ct->create_data_list ( $post );
		} else if ($post ['method'] == 'entry') {
			// 新規登録処理
			$data = $ad_ct->entry_new_data ( $post );
		} else if ($post ['method'] == 'edit_init') {
			// 編集初期処理
			$data = $ad_ct->get_detail ( $post ['edit_del_id'] );
		} else if ($post ['method'] == 'edit') {
			// 編集更新処理
			$data = $ad_ct->update_detail ( $post );
		} else if ($post ['method'] == 'delete') {
			// 削除処理
			$data = $ad_ct->delete ( $post ['id'] );
		} else if ($post ['method'] == 'recovery') {
			// 有効化処理
			$data = $ad_ct->recovery ( $post ['id'] );
		} else if ($post ['method'] == 'private') {
			// 非公開化処理
			$data = $ad_ct->private_func ( $post ['id'] );
		} else if ($post ['method'] == 'release') {
			// 公開化処理
			$data = $ad_ct->release ( $post ['id'] );
		}
	...
	private function create_data_list($post) {
		...
		$list_html = $this->ad_logic->create_data_list ( array (
		...
		$data = array (
				...
				'cate_option' => $list_html ['cate_option'],
		);
	...
	private function entry_new_data($post) {
		...
		$hi = $this->common_logic->select_logic("select * from t_ad where ad_id = ? ", array($post['dear_id']));

		// 登録ロジック呼び出し
		$this->ad_logic->entry_new_data ( array (
				$post['dear_id'],
				$hi[0]['hierarchy'] + 1,
				$post['ad'],
				$post['ad_eng'],
	...
	private function get_detail($ad_id) {
		...
		$data = array (
				'status' => true,
				'dear_id' => $reult_detail['dear_id'],
				'hierarchy' => $reult_detail['hierarchy'],
				'ad' => $reult_detail['ad'],
				'ad_eng' => $reult_detail['ad_eng'],
				'thumbnail' => $reult_detail['thumbnail'],
			);
	...
	private function update_detail($post) {
		...
		// 編集ロジック呼び出し
		$this->ad_logic->update_detail ( array (
				$post['dear_id'],
				$hi[0]['hierarchy'] + 1,
				$post['ad'],
				$post['ad_eng'],
	...
	public function recovery($id) {
		// 更新ロジック呼び出し
		$this->ad_logic->recoveryl_func ( $id );
