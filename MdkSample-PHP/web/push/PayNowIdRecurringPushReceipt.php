...
$logger->info("PayNowID課金通知データ受取開始。");

# ベリトランスペイメントゲートウェイからの課金通知電文を取得
...
if ($fp == FALSE) {
    $logger->error('課金通知データの受信に失敗しました。');
    # 読み込めないので 500 を応答
    header("HTTP/1.0 500 Internal Server Error\r\n");
    header("Content-Type: text/html\r\n\r\n");
...
if (!TGMDK_MerchantUtility::checkMessage($body, $hmac)) {
    $logger->error('課金通知データの検証に失敗しました。');
    # 改竄の疑いあり 500 を応答
    header("HTTP/1.0 500 Internal Server Error\r\n");
    header("Content-Type: text/html\r\n\r\n");
...
$pushid = $_POST['pushId'];
$file .= 'PayNowId' . '-' . $pushtime . '-' . $pushid . '.csv';
...
$logger->info("課金通知CSVファイル($file)へ書込開始。");
...
for ($i = 0 ; $i < $number_of_notify ; $i++) {
    ...
    # 課金グループID
    $key = 'chargeGroupId' . $index;
    $value = $_POST[$key];
    $buf .= QUOTATION . $value . QUOTATION;
    $buf .= SEPARATOR;

    # 課金金額
    $key = 'chargeAmount' . $index;
    $value = $_POST[$key];
    $buf .= QUOTATION . $value . QUOTATION;
    $buf .= SEPARATOR;

    # 課金実施年月日
    $key = 'chargeDate' . $index;
    $value = $_POST[$key];
    $buf .= QUOTATION . $value . QUOTATION;
    $buf .= SEPARATOR;
...
$logger->info("課金通知CSVファイル($file)書込終了。");
$logger->info("課金通知処理終了。ベリトランスペイメントゲートウェイへ 200 OK を応答。");
