...
$logger->info("ショッピングクレジット決済審査結果通知データ受取開始。");

# ベリトランスペイメントゲートウェイからの審査結果通知電文を取得
...
if ($fp == FALSE) {
    $logger->error('審査結果通知データの受信に失敗しました。');
    # 読み込めないので 500 を応答
    header("HTTP/1.0 500 Internal Server Error\r\n");
    header("Content-Type: text/html\r\n\r\n");
...
if (!TGMDK_MerchantUtility::checkMessage($body, $hmac)) {
    $logger->error('審査結果通知データの検証に失敗しました。');
    # 改竄の疑いあり 500 を応答
    header("HTTP/1.0 500 Internal Server Error\r\n");
    header("Content-Type: text/html\r\n\r\n");
...
$logger->info('審査結果通知データの検証に成功しました。');
...
$pushid = $_POST['pushId'];
$file .= 'Oricosc' . '-' . $pushtime . '-' . $pushid . '.csv';
...
$logger->info("審査結果通知CSVファイル($file)へ書込開始。");
...
for ($i = 0 ; $i < $number_of_notify ; $i++) {
    ...
    # 注文番号
    $key = 'oricoOrderNo' . $index;
    $value = $_POST[$key];
    $buf .= QUOTATION . $value . QUOTATION;
    $buf .= SEPARATOR;

    # 審査結果
    $key = 'orderStateCode' . $index;
    $value = $_POST[$key];
    $buf .= QUOTATION . $value . QUOTATION;
    $buf .= SEPARATOR;
    ...
    # 承認番号
    $key = 'approvalNo' . $index;
    $value = $_POST[$key];
    $buf .= QUOTATION . $value . QUOTATION;
    $buf .= SEPARATOR;

    # 申込日
    $key = 'requestDate' . $index;
    $value = $_POST[$key];
    $buf .= QUOTATION . $value . QUOTATION;
    $buf .= SEPARATOR;

    # ローン元金
    $key = 'loanPrincipal' . $index;
    $value = $_POST[$key];
    $buf .= QUOTATION . $value . QUOTATION;
    $buf .= SEPARATOR;

    # 支払回数
    $key = 'paymentCount' . $index;
    $value = $_POST[$key];
    $buf .= QUOTATION . $value . QUOTATION;
    $buf .= SEPARATOR;
...
$logger->info("審査結果通知CSVファイル($file)書込終了。");
$logger->info("審査結果通知処理終了。ベリトランスペイメントゲートウェイへ 200 OK を応答。");
